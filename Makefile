...
LDFLAGS          +=
MEX_CFLAGS        = CFLAGS='$$CFLAGS $(CFLAGS)' -L$(VLFEATDIR)/$(BINDIR) -lvl -I. -Itoolbox -I$(VLFEATDIR) -I$(VLFEATDIR)/toolbox
...
mex_src := $(shell find . -name "*.c")
mex_tgt := $(mex_src:.c=.$(MEX_SUFFIX))

.PHONY: all-mex
all-mex : $(mex_tgt)

toolbox/%.$(MEX_SUFFIX) : toolbox/%.c
	@echo "   MX '$<' ==> '$@'"
...
docdeep: all
	cd toolbox ; \
	matlab -nojvm -nodesktop -r 'vlg_setup;demo_all;exit'
